...
// CSM shadows: the third struct in cbFrameAttribs is the DiligentFX
// LightAttribs (ShadowMapAttribs included) filled by ShadowDiligent.cpp —
// mWorldToLightView stored TRANSPOSED (PackMatrixRowMajor = false, the
// runtime glslang convention, same as the camera matrices). The layout
// (frame, sun, light) is shared verbatim with the pixel stage.
...
struct PSTerrainIn
{
    ...
    float3 PosInLightViewSpace : LIGHT_SPACE_POS; // CSM sampling (render-space position in light view space)
    float4 PosInLightProjSpace : LIGHT_PROJ_POS;  // CSM: cascade 0 projection space (debug bypass path)
};
...
PSTerrainIn main(in VSTerrainIn In)
{
    ...
    // The terrain lattice is static — the previous-frame position differs
    // only by the camera, so reprojecting the SAME rel with the PREVIOUS
    // jittered view-proj gives the motion-vector reference (the pixel
    // shader turns the pair into an NDC delta).
    Out.PrevClipPos = mul(float4(rel, 1.0), g_Frame.PrevCamera.mViewProj);
    Out.WorldPos = In.Position;
    Out.Normal   = In.Normal;
#if ENABLE_CSM_SHADOWS
    // Render-space (anchor-relative) position into light view space — the
    // cascade distribution runs in the same camera-anchored space.
    float4 lpos = mul(float4(rel, 1.0), g_Light.ShadowAttribs.mWorldToLightView);
    Out.PosInLightViewSpace = lpos.xyz;
    // Debug bypass: cascade-0 full projection (same matrix the depth pass
    // renders with) — lets the PS sample without the scale/bias path.
    Out.PosInLightProjSpace = mul(float4(rel, 1.0),
            g_Light.ShadowAttribs.mWorldToShadowMapUVDepth[0]);
#else
    Out.PosInLightViewSpace = float3(0.0, 0.0, 0.0);
    Out.PosInLightProjSpace = float4(0.0, 0.0, 0.0, 0.0);
#endif
    return Out;
}
