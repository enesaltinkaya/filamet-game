...
void azgaarHeightmapSourceInit(AzgaarHeightmapSource* src,
    ...
    // The plateau query is backed by a lazily-built grid; build it here so it
    // is live before any tile generates (init runs before
    // heightmapTerrainInit in the load path).  Settlements data is immutable
    // after azgaarWorldLoad, so no concurrent grid-swap handling is needed.
    azgaarSettlementsPlateauInit(world);
}
}  // namespace game
