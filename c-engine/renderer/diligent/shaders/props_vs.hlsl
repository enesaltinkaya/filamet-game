...
// CSM shadows: cbFrameAttribs layout shared verbatim with the pixel stage
// (frame, sun, DiligentFX LightAttribs — mWorldToLightView TRANSPOSED,
// filled by ShadowDiligent.cpp).
...
PSPropsIn main(in VSPropsIn In, uint InstId : SV_InstanceID)
{
    ...
    // The wind-swayed position is reused for the previous frame (the
    // per-frame sway delta is sub-pixel — the camera dominates the motion
    // vector); reprojected with the PREVIOUS jittered view-proj.
    Out.PrevClipPos = mul(float4(worldRel, 1.0), g_Frame.PrevCamera.mViewProj);
    ...
    // Render-space (anchor-relative) position into light view space.
    float4 lpos = mul(float4(worldRel, 1.0), g_Light.ShadowAttribs.mWorldToLightView);
