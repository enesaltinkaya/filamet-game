...
using engine::renderer::diligent::diligentCreateImageTexture;
...
constexpr float kCullYMin   = -500.0f;
constexpr float kCullYMax   = 9000.0f;
constexpr float kCullReach  = 64.0f;

// Default terrain textures (engine pak ktx2): basis sources transcode to
// BC7 through c-utils — the DFD's sRGB flag picks BC7_SRGB for albedos
// (hardware decode-on-sample, the same colouring the PNG path's IsSRGB
// gave) and BC7_UNORM for normals — and the baked 11-mip chain uploads as
// compressed blocks: no GPU mipgen, 1.4 MB instead of 5.6 MB per texture.
// The PNG twins in the game pak stay as provenance
// (scripts/unpack-terrain-ktx2.sh, docs/lessons.md 2026-09).
// Albedos must sample sRGB so the hardware decode matches the
// CPU-composited look.
...
struct GpuTile {
    ...
    i32           tileX = 0, tileZ = 0;
    u64           readyStamp = 0;
    float         originX = 0.0f, originZ = 0.0f, sizeMeters = 0.0f;
    IBuffer*      vbo = nullptr;
};
...
// CSM shadow pass state (DiligentFX ShadowMapManager; ShadowDiligent.cpp
// owns the atlas + cascade distribution, this pass owns its depth-only
// pipeline and the tile draw loop). Rebuilt when the shadow settings
// generation moves (mode/quality change — the sampling macros are
// compile-time) alongside the lit pipeline.
IShader*               shadowVs = nullptr;
IPipelineState*        shadowPipeline = nullptr;
IPipelineResourceSignature* shadowPrs = nullptr;
IShaderResourceBinding* shadowSrb = nullptr;
...
// ktx2: c-utils decodes (basis → BC7/BC5/BC4 by DFD channel count, the DFD's
// sRGB flag picks the SRGB block variants) and diligentCreateImageTexture
// uploads the compressed blocks with their baked mip chain — no GPU mipgen,
// a quarter of the RGBA8 decode path's VRAM/bandwidth. PNG (the provenance
// twins, mip 0 only) still goes through the DiligentTools TextureLoader
// (stb decode + GenerateMips).
ITexture* loadKtx2Texture(const char* path, bool srgb) {
    if (utils::strEndsWithC(path, "ktx2") || utils::strEndsWithC(path, "ktx")) {
        utils::Image image = utils::imageLoad(path);
        if (!image.data || !image.isKtx || image.width <= 0 || image.height <= 0) {
            utils::warn("heightmapTerrain: ktx2 decode failed: %s", path);
            if (image.data) {
                utils::imageDestory(&image);
            }
            return nullptr;
        }
        ITexture* tex = diligentCreateImageTexture(image, path, srgb);
        utils::imageDestory(&image);
        return tex;
    }
...
void rebuildSharedSRB(void) {
    ...
    prs->CreateShaderResourceBinding(&newSrb, true);
    if (!newSrb) {
        utils::warn("heightmapTerrain: SRB creation failed");
        return;
    }
    // The CSM shadow map is a MUTABLE resource — bind per SRB (the
    // DiligentFX PBR_Renderer pattern; the view carries the comparison
    // sampler for the combined-image-sampler resolution).
    if (engine::renderer::diligent::shadowDiligentActive()) {
        ...
        if (ITextureView* srv = engine::renderer::diligent::shadowDiligentShadowSRV()) {
            if (IShaderResourceVariable* v = newSrb->GetVariableByName(SHADER_TYPE_PIXEL, texName))
                v->Set(srv);
        }
    }
...
void initConstantBuffers(void) {
    // PBRFrameAttribs + one PBRLightAttribs (the sun) + one DiligentFX
    // LightAttribs (CSM shadows) + the shadow receiver fade (f4ShadowFade),
    // the layout materials/* share verbatim.
...
// The lit PSO/PRS + the CSM shadow depth PSO. Both are rebuilt whenever the
// shadow settings generation moves (mode/quality change): the sampling
// macros (ENABLE_CSM_SHADOWS/SHADOW_MODE/PCF_FILTER_SIZE/...) are
// compile-time and the PRS resource list must match the active mode's
// shader resources (raw depth + comparison sampler for PCF, filterable
// texture + linear sampler for VSM/EVSM, none when shadows are off).
void destroyLitPipeline(void);

void litMacros(ShaderMacroHelper& m) {
    using engine::renderer::diligent::shadowDiligentActive;
    using engine::renderer::diligent::shadowDiligentGeneration;
    const bool active = shadowDiligentActive();
    m.AddShaderMacro("ENABLE_CSM_SHADOWS", active ? 1 : 0);
    if (active) {
        // Mode + filter knobs mirror the ShadowDiligent.cpp tier table (the
        // getters keep the passes decoupled from the manager's internals).
        ...
        m.AddShaderMacro("SHADOW_DEBUG_VIS", getenv("ENGINE_SHADOW_DEBUG_VIS") ? atoi(getenv("ENGINE_SHADOW_DEBUG_VIS")) : 0);
    }
}
...
bool buildShadowPipeline(void) {
    ...
    shadowVs = createHlslShader("materials/heightmap_terrain_shadow_vs.hlsl",
            "heightmapTerrainShadowVS", SHADER_TYPE_VERTEX);
    ...
    if (!shadowPrs) {
        utils::warn("heightmapTerrain: shadow PRS creation failed");
        return false;
    }
    ...
    shadowPrs->CreateShaderResourceBinding(&shadowSrb, true);
    if (!shadowSrb) {
        utils::warn("heightmapTerrain: shadow SRB creation failed");
        return false;
    }

    GraphicsPipelineStateCreateInfo psoCI;
    psoCI.PSODesc.Name             = "heightmapTerrainShadow";
    psoCI.ppResourceSignatures    = &shadowPrs;
    psoCI.ResourceSignaturesCount = 1;

    GraphicsPipelineDesc& gp = psoCI.GraphicsPipeline;
    // Depth-only: no pixel stage, no render targets. Nothing is culled —
    // the light view's winding flips with the sun azimuth, and a
    // heightfield has no overhangs to benefit from culling.
    ...
    if (!shadowPipeline) {
        utils::warn("heightmapTerrain: shadow PSO creation failed");
        return false;
    }
    return true;
}
...
bool buildLitPipeline(void) {
    ...
    for (const PipelineResourceDesc& r : gResources) resources.push_back(r);
    ...
    if (engine::renderer::diligent::shadowDiligentActive()) {
        ...
        if (pcf) {
            ...
            // The comparison sampler rides as an IMMUTABLE sampler (the
            // DiligentFX PBR_Renderer shadow pattern — a standalone SAMPLER
            // resource next to the view-attached sampler crashes the Vulkan
            // backend's descriptor generation).
...
void destroyShadowPipeline(void) {
    if (shadowSrb) { shadowSrb->Release(); shadowSrb = nullptr; }
...
// Poll the shadow settings generation (bumped on any mode/quality change):
// rebuild both pipelines when it moved. Cheap enough to run every frame.
void checkShadowGeneration(void) {
    ...
    utils::info("terrainShadowsDbg: rebuild begin gen=%u srb=%p", gen, (void*)srb);
    destroyLitPipeline();
    destroyShadowPipeline();
    if (!buildLitPipeline() || !buildShadowPipeline()) {
        utils::warn("heightmapTerrain: shadow-generation pipeline rebuild failed");
        initFailed = true;
        passReady = false;
        return;
    }
    utils::info("heightmapTerrain: pipelines rebuilt (shadow generation %u) srb=%p shadowPipeline=%p",
            gen, (void*)srb, (void*)shadowPipeline);
}
...
void fillFrameAttribs(void) {
    ...
    // the VS' prev-clip position (the terrain is static geometry, so the
    // previous position differs only by the camera). PrevCamera matrices
    // TRANSPOSED like the current ones above (runtime glslang convention).
            ...
            engine::renderer::diligent::taaCurrentJitterY()};
    {
        ...
        // Prev-camera anchor correction (taaPrevAnchorDelta): the VS reuses
        // the CURRENT anchor's rel for PrevClipPos; fold the per-frame camera
        // translation in so the motion vectors keep the translation flow.
        // The uploaded (transposed) matrix is consumed column-vector-style
        // (glslang column-major cbuffer packing), so the translation goes in
        // its 4th COLUMN — post-multiply (see TaaDiligent.h).
        f32 dEye[3];
    ...
    // Shadow receiver fade (f4ShadowFade): the cascade box reaches well past
    // the tier's shadow distance (StabilizeExtents bounding sphere of the
    // 2.8x caster-padded frustum), so the PS fades the shadow to lit by
    // .x (view-space m) starting at .y. Zeroed above while shadows are off.
...
void drawImpl(void) {
    ...
    fillFrameAttribs();
    if (!srb || !pipeline) {
        utils::warn("terrainShadowsDbg: null state srb=%p pipeline=%p", (void*)srb, (void*)pipeline);
        return;
    }
...
// The shadow pass' per-cascade depth draw (called by
// shadowDiligentRenderCascades with the cascade DSV bound and the shared
// cbShadowPass filled). Mirrors drawImpl's tile loop with the depth-only
// PSO.
void heightmapTerrainDiligentShadowDraw(const FrustumCullPlanes* planes, const f32 camPos[3]) {
    heightmapTerrainDiligentShadowDrawPSO(nullptr, planes, camPos);
}

void heightmapTerrainDiligentShadowDrawPSO(void* psoOverride, const FrustumCullPlanes* planes,
        const f32 camPos[3]) {
    IPipelineState* pso = (IPipelineState*)psoOverride;
    if (!passReady || !(pso ? pso : shadowPipeline) || !shadowSrb) return;

    context->SetPipelineState(pso ? pso : shadowPipeline);
    context->CommitShaderResources(shadowSrb, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
    context->SetIndexBuffer(latticeIbo, 0, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);

    static const bool dbgCull = getenv("ENGINE_SHADOW_CULL_DEBUG") != nullptr;
    u32 dbgTotal = 0, dbgKept = 0;
    for (GpuTile& t : gpuTiles) {
        if (!t.inUse) continue;
        dbgTotal++;
        if (planes) {
            const float bmin[3] = {t.originX - kCullReach - camPos[0], kCullYMin - camPos[1],
                    t.originZ - kCullReach - camPos[2]};
            const float bmax[3] = {t.originX + t.sizeMeters + kCullReach - camPos[0],
                    kCullYMax - camPos[1],
                    t.originZ + t.sizeMeters + kCullReach - camPos[2]};
            if (frustumCullAabbOutside(bmin, bmax, *planes)) continue;
        }
        dbgKept++;
        IBuffer* vbo = t.vbo;
        context->SetVertexBuffers(0, 1, &vbo, nullptr,
