...
// The CSM shadow pass' per-cascade instanced depth draw (ShadowDiligent.cpp
// calls it once per cascade). No-op while the pass is not initialized.
void propsRenderDiligentShadowDraw(const renderer::diligent::FrustumCullPlanes* planes,
        const f32 camPos[3]);
// The current wind/player state (the lit fillFrameAttribs packs into
// f4ExtraData[0..2]; the shadow pass' cbuffer needs the same values): wind
// = (dir.xy, speed, gust strength), phase = (integrated sway phase rad),
// player = (feet.xyz, horizontal speed).
