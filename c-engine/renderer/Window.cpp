...
namespace engine {
    ...
    // built into SDL color cursors; text stays a system cursor. The pointers are handed to the
    // crmlui wrapper; cursorVisible is tracked so windowIsCursorVisible() can gate GUI input
    // forwarding.
    ...
    bool windowCreate(const char* title, u32 width, u32 height) {
        ...
        if (width == 0 || height == 0) {
            ...
            height          = 720;
            SDL_Rect bounds = {};
            if (SDL_GetDisplayBounds(SDL_GetPrimaryDisplay(), &bounds) && bounds.w > 0 &&
                bounds.h > 0) {
        ...
        // SDL3: no position params (window is centered), shown by default, no ALLOW_HIGHDPI (always
        // on)
    ...
    void windowPollEvents(void) {
        ...
        for (int i = 0; i < fakeEscN; i++)
            if (fakeEsc[i] == utils::timer.frameCounter) {
                ...
                utils::debug("ESC-DEBUG fake esc injected frame=%llu",
            ...
            switch (event.type) {
                ...
                case SDL_EVENT_KEY_DOWN:
                    ...
                    if (event.key.scancode == SDL_SCANCODE_ESCAPE)
                        utils::debug("ESC-DEBUG SDL KEY_DOWN esc (non-repeat) frame=%llu",
                                     (unsigned long long)utils::timer.frameCounter);
                ...
                case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED:
                    if (window.width != (u32)event.window.data1 ||
    ...
    static SDL_Cursor* loadCursorImage(const char* path, float xHot, float yHot) {
        ...
        SDL_Surface* surface =
            SDL_CreateSurfaceFrom(w, h, SDL_PIXELFORMAT_RGBA32, resized.data, 4 * w);
    ...
    void windowSetCursor(int cursorType) {
        ...
        switch (cursorType) {
            ...
            case 2:
                ...
                break;  // text
            default:
                break;  // 0=arrow (and any unhandled)
        }
