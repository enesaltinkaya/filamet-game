...
static const float CAPSULE_HALF_HEIGHT = 0.45f;
static const float CAPSULE_RADIUS      = 0.25f;
static const float TP_LOOK_AT_HEIGHT   = 1.1f;
static const float MAX_SLOPE_ANGLE     = 45.0f * (float)M_PI / 180.0f;
static const float RUN_SPEED           = 4.0f; // m/s
static const float WALK_SPEED          = 2.0f; // shift-held
static const float JUMP_SPEED          = 4.0f;
static const float SPRINT_MULT         = 40.0f;
...
static const f32 TURN_SPEED = 20.0f;
...
static void playerUpdateMouseMode(void) {
    ...
    if (drag && !p.dragging) {
        if (guiManagerIsMouseInteracting()) return;
        p.dragging = 1;
