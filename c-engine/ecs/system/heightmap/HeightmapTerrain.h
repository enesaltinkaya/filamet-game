 ...
 *
 * The game owns one HeightmapTerrain per world (plain struct, one instance
 * per world). heightmapTerrainSystem tracks the streaming window around the
...
// Configure the terrain (idempotent: frees any previously resident tiles).
...
// Free all resident tile data. The struct itself is owned by the game
// (e.g. until the world is destroyed).
...
// Deterministic CPU self-test (the game gates this behind ENGINE_HEIGHTMAP_TEST=1).
// Drives the real streaming path with an analytic world-anchored source:
// init a temporary instance (3x3 window, 1022 m tiles so the 2 m grid step is
// exact in f32) -> updateWindow -> wait (bounded) for all window tiles READY ->
// check snapshot count == window^2, heightmapTerrainSample == bilinear of the
// resident grid (and == the source for an absent tile), adjacent READY tiles
// share bit-identical border columns, then evict (re-anchor far away),
// regenerate, and check the grid is bit-identical to the first generation.
// Logs a single PASS/FAIL summary line. Returns true on PASS. Main thread.
