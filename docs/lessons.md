...
## 2026-09-05 — Mouse look ~N× slower at unlimited FPS: `input.mouseDx/Dy` was reset in every rendered frame's poll, but consumed only by the fixed 60 Hz simulation tick — every delta that landed on a non-tick frame was wiped

**Rule:** a value that is accumulated per rendered frame but consumed by a fixed-timestep system must NOT be reset per rendered frame — the reset wipes all accumulation except the last frame's, and whenever the render fps exceeds the tick rate the consumer starves (rotation/zoom speed ∝ tick rate / render fps). Accumulate across frames and zero it in the consuming tick instead (consumer-side reset, as the old engine did with `playerInput.mouseDx`). The converse holds for per-frame-only values (key presses, the GUI event stream): keep them fresh per frame. `utils::timer.dt` is the fixed 1/UPS tick (1/60), not the frame's dt — never read it as "frame time"; the frame's wall time is `utils::timer.frameTime` (NANOS, clamped 250 ms). Exactly one system may consume+zero a shared input field per tick — here the player camera (`p.dragging`) and the flying camera (`flying`) are mutually exclusive by construction (the player's update early-returns in fly mode), which is what makes consumer-side reset safe.

**Incident:** "when fps is unlimited, mouse look rotation is very slow". The port moved the old engine's consumer-side reset (`playerInput.mouseDx = 0` after the tick's consumption) into `windowPollEvents` (per rendered frame: `input.mouseDx = 0; += SDL relative delta`). At 60 fps (1 tick : 1 frame) the two coincide, so nothing was wrong there; at any render fps above the 60 Hz sim tick, each poll zeroes the buffer before the tick reads it, so only 1 of every N frames' mouse motion survived (N = render fps / 60) → look N× slower. The `* utils::timer.dt` factor in the rotation was a red herring (dt is the constant 1/60 — the same formula the old engine used; its feel came from the consumer-side reset, not the dt).

**Fix:** `c-engine/renderer/Window.cpp` — `windowPollEvents` no longer resets `mouseDx/Dy` (accumulation block only); `windowSetRelativeMouseMode(0)` zeroes them so a finished drag can't leave a stale buffer behind. `c-engine/ecs/system/player/Player.cpp` + `flyingCamera/FlyingCamera.cpp` — each consumer zeroes the buffer after applying (player gated by `p.dragging`, after the sky-look block which is the last reader). `CAM_SENS` is now `0.15f / 60.0f` rad/px — the old `0.15 * dt` with the fixed tick inlined, so the 60 fps feel is bit-identical to the old engine and to the previous 60 fps-capped new-engine behaviour.

**Verification:** temporary hooks (both removed): the `ENGINE_FAKE_DRAG` feed modelled a real mouse at constant 600 px/s (`dx += 600 * frameTime/1e9` — frameTime is nanos; a per-frame constant feed would have masked the bug, since it makes the old formula look frame-rate-independent), and a yaw log every 300 ticks in `playerUpdateCamera`. Matrix (yaw rate over 5 s windows): pre-fix unlimited ≈ 0.1–0.27 rad/s; post-fix 60 fps cap 1.50 rad/s; post-fix unlimited 1.50 rad/s (= 600 px/s × 0.0025 rad/px exactly, and = the old engine's 60 fps feel). Screenshot boot clean.
