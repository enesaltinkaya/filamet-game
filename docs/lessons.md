...
- Automated runs must never write the state db: menu-enter (ENGINE_AUTOTEST=enter) ends with the orbit ACTIVE and the character sliding, and the 1 Hz periodic saves overwrite the parked `player` + `camera` rows (fingerprint: `flying camera: loaded saved state pos` with coordinates you never parked) — gated with `automatedRun()`, rows restored from a pre-session byte dump; "shadow invisible" at a known vantage was contrast, not the caster (IBL-dominated: direct sun ≈ +20%, shadow ≈ 10% sRGB dip) — fixed with ambient-in-shadow darkening (`f4ShadowFade.w`, `ENGINE_SHADOW_AMBIENT_DARKEN`, default 0.5); `ENGINE_CAMERA_DB=1` opts automated runs back into the parked camera
...
- Sliced CSM casting erases tall-distant-caster shadows near the camera: ground inside cascade 0's slice samples an atlas the far-away tall caster never rendered into — its shadow is cut out in a disc around the camera (the focus band made the boundary sweep with orbit distance) — a receiving cascade must cast the full tier, but collapsing to 1 cascade was reverted (player shadow unreadable at ~4 texel/m) — landed fix: per-pixel composition in the splat PS (sample every cascade with viewZ ≤ its zEnd and UV in-box, multiply attenuations, fade once); PBR model single-pick caveat pending; diagnose with `ENGINE_SPLAT_SHADOW_DEBUG=3` (unsampled/lit strip) + `ENGINE_SHADOW_ORACLE` (passes → caster absent, not a math bug)
